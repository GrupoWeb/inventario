<template >
  <div>
    ...
    <el-main>
      <label><b>Total: </b>{{ totalArray }}</label>
      <!-- <el-table :data="da"> -->
      <el-table
        :data="
          list_response.list_Check.slice(
      ...
      >
        ...
        <el-table-column prop="ingreso" label="INGRESADO" width="100">
        </el-table-column>
      </el-table>
...
export default {
  data() {
    return {
      ...
      urlData: {
        urlEntity: "entidades",
        urlSat:
          "http://gestorquejas.diaco.gob.gt/Consulta/rs/proveedores/empresa?nit=",
        barCode: "barCode",
        unidades: "unidades",
        grupos: "grupos",
  ...
  methods: {
    ...
    onSubmit(form) {
      ...
      this.$refs[form].validate((valid) => {
        if (valid) {
          ...
          axios
            .post(this.urlData.setExcel, {
              UE: this.form.unidad,
              CUENTA: this.form.cuenta_nueva,
              data_excel: this.list_response.list_Check,
            })
            .then((response) => {
    ...
    getUnidad() {
      axios.get(this.urlData.unidades).then((response) => {
        this.list_response.list_unidad = response.data;
      });
    },
    getCuentas() {
      axios.get(this.urlData.cuentas).then((response) => {
    ...
    importfxx(obj) {
      ...
      // axios.post(this.urlData.getBienes,{
      //     cuenta: this.form.cuenta_nueva,
      ...
      FileReader.prototype.readAsBinaryString = function (f) {
        ...
        reader.onload = function (e) {
          ...
          for (let data = 0; data < arr.length; data++) {
            axios
              .post(_this.urlData.getBienes, {
                cuenta: _this.form.cuenta_nueva,
                unidad: _this.form.unidad,
                codigo: arr[data].bien,
              })
              .then((response) => {
                console.log(arr[data]);
                // console.log('response',response.data.length);
                if (response.data.length === 0) {
                  _this.list_response.list_Check.push({
                    fecha: arr[data].fecha,
                    bien: arr[data].bien,
                    descripcion: arr[data].descripcion,
                    cantidad: arr[data].cantidad,
                    costo: arr[data].costo,
                    alza: arr[data].alza,
                    ingreso: "si",
                  });
                }
            ...
            //     if(response.data[index].codigo_sicoin === arr[data].bien){
            ...
            //         })
            //     }else{
            //         console.log('DB: ', response.data[index].codigo_sicoin, ' Archivo: ', arr[data].bien)
            //         _this.list_response.list_Check.push({
            //             'fecha': arr[data].fecha,
            //             'bien' : arr[data].bien,
            //             'descripcion' : arr[data].descripcion,
          ...
          _this.total = _this.list_response.list_Check.length;
          _this.list_response.list_excel = arr;
          _this.submit_button = false;
          _this.totalArray = _this.list_response.list_Check.length;
          return arr;
        };
