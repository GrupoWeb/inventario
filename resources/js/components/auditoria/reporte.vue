...
export default {
  data() {
    return {
      ...
      url: {
        ...
        unidades: "unidades",
        BarCodeAllReportAuditoria: "BarCodeAllReportAuditoria"
      },
      show_table: true,
  ...
  methods: {
    ...
    getUnidad() {
      axios.get(this.url.unidades).then((response) => {
        this.unidadesItem = response.data;
      });
    },
    getBienesAll() {
      this.bien = [];
      axios
        .post(this.url.BarCodeAllReportAuditoria, {
          account: this.select_item,
          unidad: this.select_unidad
        })
        .then((response) => {
