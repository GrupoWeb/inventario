        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="150px">
          ...
          <el-form-item>
            <!-- <el-link
              :underline="false"
              target="_black"
              v-bind:href="'/printCode/' + form.name"
            >
              <el-button type="primary" icon="el-icon-search"> </el-button>
            </el-link> -->
            <el-button type="primary" icon="el-icon-search" @click="pdfBar()">
            </el-button>
          </el-form-item>
        </el-form>
      </div>
...
export default {
  ...
  methods: {
    ...
    showBarCode() {
      ...
      axios.get(this.urlData.pdfCode + this.form.name).then((response) => {
        if (response.data === true) {
          this.drawer = true;
          this.src = "./../codigo_barra/" + this.form.name + ".pdf";
        } else {
          this.$message.error({
            message: h("p", null, [
              h("i", { style: "color: red" }, "No existe el código SICOIN"),
            ]),
          });
        }
      });
